/*
 * Editor: Sierra Andersen
 * Date: 4 Jan 2023
 * 
 * This is an edited version of the Sierpinski Triangle program.
 * This program allows the user to change the order with buttons.
 */
...
public class SierpinskiTriangle extends Application {
	...
	public void start(Stage primaryStage) {       
		...
		//Buttons to increase/decrease the order.
		Button btDown = new Button("-");
		Button btUp = new Button("+");
		
		//Button decreases the order.
		btDown.setOnAction(
				e->{
					if(trianglePane.order > 0)
						trianglePane.order--;
						trianglePane.setOrder(trianglePane.order);
				}
				);
		
		//Button increases the order.
		btUp.setOnAction(
				e->{
					trianglePane.order++;
					trianglePane.setOrder(trianglePane.order);
